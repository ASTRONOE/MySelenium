...
class AirPeaceDriver(webdriver.Edge):
  ...
  def departure_date(self, day:int, mon:int|str, year:int, id:str):
    ...
    calendar = self.find_element(By.CSS_SELECTOR,'table.table-condensed')
    time.sleep(2)

    month_drop = Select(calendar.find_element(By.CSS_SELECTOR, 'select.monthselect.dropdown-scroll-open'))
    if type(mon) is int:
      mon -= 1
      #month =
      month_drop.select_by_value(str(mon))
    elif type(mon) is str:
      month_drop.select_by_visible_text(mon)
    #self.action.move_to_element(month).click()

    calendar = self.find_element(By.CSS_SELECTOR,'table.table-condensed')
    time.sleep(2)
    year_drop = Select(calendar.find_element(By.CSS_SELECTOR, 'select.yearselect.dropdown-scroll-open'))
    year_drop.select_by_value(str(year))
  ...
  def transaction(self, block, cell):
    desktop_fare = self.find_elements(By.CSS_SELECTOR, 'div.desktop-fare-block')[block]
    block_choice = desktop_fare.find_elements(By.TAG_NAME, 'a')[cell]
    self.action.move_to_element(block_choice).click().perform()
    time.sleep(2)

    big_box = self.find_element(By.CSS_SELECTOR, 'div.branded-fare-type-column.col-lg-4')
    price_click = big_box.find_element(By.CSS_SELECTOR, 'a.button.ui-button__type-4.mt-0')
    self.action.move_to_element(price_click).click().perform()
    time.sleep(2)

    button_block = self.wait.until(visibility((By.CSS_SELECTOR, 'div.continue-btn-block')))
    continue_transaction = button_block.find_element(By.ID, 'continueButton')
    self.action.move_to_element(continue_transaction).click().perform()
...
if __name__ == "__main__":
  ...
  firstflight.execute_script("window.scrollBy(0, 80);")
  # time.sleep(3)
  # firstflight.departure_date(28, "Nov", 2024, 'roundTripDepartureDate')
  time.sleep(3)
  firstflight.passenger_count()
  firstflight.departure_location("Lagos")
  time.sleep(3)
  firstflight.departure_location("Portharcourt", "firstArrPort")
  time.sleep(3)
  firstflight.departure_date(1, "Oct", 2024, 'roundTripDepartureDate')
  # time.sleep(3)
  firstflight.departure_date(9, "Aug", 2024, 'oneWayDepartureDate')
  time.sleep(3)
  firstflight.search_flight_click()
  time.sleep(3)
  firstflight.transaction(1, 0)
